...
export interface PublishedDateRepairInput {
  url: string;
  published_at: string;
  published_date_jst: string;
}

export interface PublishedDateRepairResult {
  total: number;
  repaired: number;
  errors?: any[];
}
...
/**
 * 既存記事の公開日をフィードの公開日時へ補正する UPDATE 文を組み立てる。
 * 登録済みの公開日時より古い公開日時が判明した行のみを対象とし、
 * 日付が後ろへ動くことは無いようにする。
 */
export function buildPublishedDateRepairStatement(articles: PublishedDateRepairInput[]): {
  sql: string;
  params: string[];
} {
  const valuePlaceholders = articles.map(() => "(?, ?, ?)").join(", ");
  const params: string[] = [];
  for (const article of articles) {
    params.push(article.url, article.published_at, article.published_date_jst);
  }

  const sql = `
WITH fixes(url, published_at, published_date_jst) AS (VALUES ${valuePlaceholders})
UPDATE articles
SET published_at = fixes.published_at,
    published_date_jst = fixes.published_date_jst
FROM fixes
WHERE articles.url = fixes.url
  AND articles.published_at > fixes.published_at;
`.trim();
...
/**
 * 収集時刻ベースなどで誤った公開日が登録されている既存記事を、
 * フィードが提供する本来の公開日時へ補正する（スコアリングや再ベクトル化は行わない）
 */
export async function repairPublishedDatesInD1(
  options: Pick<D1SyncOptions, "accountId" | "databaseId" | "apiToken" | "customFetch"> & {
    articles: PublishedDateRepairInput[];
    batchSize?: number;
  },
): Promise<PublishedDateRepairResult> {
  const { accountId, databaseId, apiToken, articles, batchSize = 50 } = options;

  if (!accountId || !databaseId || !apiToken || !articles || articles.length === 0) {
    return { total: articles?.length ?? 0, repaired: 0 };
  }
  ...
  for (let i = 0; i < articles.length; i += batchSize) {
    ...
    const { sql, params } = buildPublishedDateRepairStatement(chunk);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        errors.push({ message: errorText || `HTTP ${response.status}`, status: response.status });
        continue;
      }
      ...
      if (resData.success === false) {
        errors.push(
          ...(resData.errors ?? [{ message: "公開日補正が success=false を返しました" }]),
        );
        continue;
      }

      repaired += Number(resData?.result?.[0]?.meta?.changes ?? 0);
    } catch (err: any) {
