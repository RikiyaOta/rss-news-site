...
describe("ローカル多言語埋め込みスコアリングモジュール (src/pipeline/scorer)", () => {
  ...
  describe("calculateScoreFromSimilarity - 類似度からスコアへのスケーリング", () => {
     ...
     * 変更するときは `pnpm rescore --dry-run` の分布を根拠にすること。
     */
    it.each([
      // 類似度,  期待スコア, 意図
      [1.0, 100, "上限"],
      [0.8, 93, "最上位区分の中央"],
      [0.6, 85, "最上位区分の下端"],
      [0.5999, 84, "最上位区分のすぐ下 (上位区分の上端)"],
      [0.59, 81, "上位区分の上端付近"],
      [0.53, 65, "上位区分の下端"],
      [0.5299, 64, "上位区分のすぐ下 (中位区分の上端)"],
      [0.52, 61, "中位区分の上端付近"],
      [0.46, 40, "中位区分の下端"],
      [0.4599, 39, "中位区分のすぐ下 (最下位区分の上端)"],
      [0.45, 34, "最下位区分の上端付近"],
      [0.42, 19, "最下位区分の中央"],
      [0.38, 0, "最下位区分の下端"],
      [0.35, 0, "下端より低い値は 0 でクリップ"],
      [0.0, 0, "ゼロ"],
    ...
    /**
     * 実コーパス 2237 件を再スコアリングして観測した類似度を、
     * そのまま表にして固定する。区分を上へ動かすとこれらが軒並み
     * 下の帯に潰れ、「全記事が 50 点未満」の状態が再発する。
     *
     * 修正前の区分では 50 点に 0.76、実測後の最初の修正でも 0.55 を
     * 要求していたが、実際の上限は 0.56 前後しかなかった。
     */
    it.each([
      [0.561, "Rust に移植された Bun 1.4 ↔ Rust (実測の最上位)", 65, 85],
      [0.548, "分散DB の解説記事 ↔ 分散システム", 65, 85],
      [0.509, "TanStack + Hono + Cloudflare ↔ Cloudflare Workers", 40, 65],
      [0.44, "平均的な記事", 15, 40],
      [0.32, "Belgian car salesman becomes prince (完全に無関係)", 0, 0],
    ])(
      "実測された類似度 %s (%s) のスコアが %s〜%s 点の帯に収まること",
    ...
    /**
     * 「関心テーマど真ん中の記事が 50 点を超える」ことが今回の修正の目的。
     * 実測上位の類似度でこれが成り立たなくなったら区分がずれている。
     */
    it("実測された上位記事の類似度が 50 点を超えること", () => {
      for (const similarity of [0.561, 0.548, 0.546, 0.523, 0.516]) {
        expect(calculateScoreFromSimilarity(similarity, false)).toBeGreaterThan(50);
      }
    });
